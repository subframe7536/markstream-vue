...
import type { CustomComponentDisplayMode, CustomComponentMap, MarkstreamSolidComponent } from '../customComponents'
...
export interface ResolvedCustomHtmlTag {
  ...
  component: MarkstreamSolidComponent | null
...
export function resolveCustomHtmlTag(
  ...
  const component = isWhitelisted
    ? (customComponents[tag] ?? customComponents[normalizedType] ?? null)
    : null
